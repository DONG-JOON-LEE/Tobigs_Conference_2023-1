...
from Tobigs.Preprocessing.preprocessing import PreProcessing
from Tobigs.Preprocessing.feature_extractor import FeatureExtractor
np.random.seed(999)


class Dataset:
    def __init__(self, clean_filenames, noise_filenames, train, **config):
        ...
        self.train = train
        self.preprocessing = PreProcessing(**config)
    ...
    def get_noisy_audio(self, *, filename):
        return self.preprocessing.read_audio(filename, self.sample_rate)

    def _audio_random_crop(self, audio, duration):
        audio_duration_secs = librosa.core.get_duration(y=audio, sr=self.sample_rate)
    ...
    def parallel_audio_processing(self, clean_filename):
        if self.train:
            clean_filename = os.path.join('./Clean_audio_train/',clean_filename)
        else:
            clean_filename = os.path.join('./Clean_audio_val/', clean_filename)

        clean_audio, _ = self.preprocessing.read_audio(clean_filename, self.sample_rate)
        ...
        noise_filename = os.path.join('./noise/',noise_filename)
        noise_audio, sr = self.preprocessing.read_audio(noise_filename, self.sample_rate)
        noise_audio = noise_audio * 0.5
    ...
    def create_tf_record(self, *, prefix='train', subset_size, parallel=True):
        counter = 0
        for i in range(0, len(self.clean_filenames), subset_size):
            ...
            if parallel:
                out = map(self.parallel_audio_processing, clean_filenames_sublist)
            else:
            ...
            for o in out:
                ...
                noise_stft_mag_features = self.preprocessing.prepare_input_features(noise_stft_magnitude, numSegments=8, numFeatures=129)
                ...
                for x_, y_, p_ in zip(noise_stft_mag_features, clean_stft_magnitude, noise_stft_phase):
                    ...
                    example = self.preprocessing.get_tf_feature(x_, y_, p_)
